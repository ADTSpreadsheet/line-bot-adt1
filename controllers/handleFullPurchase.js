...
async function submitStarterSlip(req, res) {
  ...
  try {
    ...
    const slipFileName = `SP-${ref_code}-${Date.now()}.jpg`; // ตั้งชื่อตาม pattern เดิม
    const username = `ADT-${ref_code}`;
    const password = serial_key;

    // ✅ จัดการ base64 prefix ตาม pattern เดิม
      ...
      // การทำงานที่ 1: อัปโหลดรูปภาพ
      {
        ...
        execute: async () => {
          const uploadResult = await uploadBase64Image({
            base64String: processedFileContent, // ใช้ processedFileContent แทน
