...
const setupUsername = async (req, res) => {
  try {
    ...
    // STEP 2: ตรวจสอบ Username ซ้ำแบบ exact match
    ...
    if (authError) {
      ...
      return res.status(500).json({
        message: 'เกิดข้อผิดพลาดขณะอัปเดต auth_sessions',
    ...
    if (licenseError) {
      ...
      return res.status(500).json({
        message: 'เกิดข้อผิดพลาดขณะอัปเดต license_holders',
