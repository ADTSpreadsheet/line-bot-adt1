...
function generateRefCode() {
  // สร้างรหัสแบบสุ่ม 6 หลัก (เพิ่มความปลอดภัย)
  return Math.floor(100000 + Math.random() * 900000).toString();
}

// ฟังก์ชันสำหรับสร้าง Serial Key แบบสุ่ม
function generateSerialKey() {
  const serialNumber = Math.floor(1000 + Math.random() * 9000).toString(); // สุ่มตัวเลข 4 หลัก
  const serialChars = Math.random().toString(36).substring(2, 6).toUpperCase(); // สุ่มตัวอักษร 4 หลัก (เพิ่มจาก 2 หลัก)
  return serialNumber + "-" + serialChars; // เพิ่มขีด "-" เพื่อให้อ่านง่ายขึ้น
}
...
app.post('/webhook', express.json(), async (req, res) => {
  try {
    ...
    // เลือก Bot ตาม destination ของ event แรก (ทุก events ในชุดเดียวกันมาจาก Bot เดียวกัน)
    const destination = events[0].destination;
    let botId = null;
    let isValid = false;
    
    // ตรวจสอบว่าเป็น Bot ตัวไหนและตรวจสอบลายเซ็น
...
// ฟังก์ชันจัดการการขอ Ref.Code (รวมฟังก์ชันของทั้งสอง Bot)
async function handleRefCodeRequest(userId, messageText, lineClient, botId) {
  try {
    ...
    // ข้อความที่จะส่งกลับแตกต่างกันเล็กน้อยตาม Bot
    ...
    // ส่ง Ref.Code กลับไปให้ผู้ใช้ทาง LINE
...
app.post('/verify-ref-code', express.json(), async (req, res) => {
  try {
    ...
    const { data: refCodeData, error } = await supabase
      ...
      .eq('ref_code', refCode)
      .eq('status', 'PENDING')
      .gt('expires_at', new Date().toISOString()) // ตรวจสอบว่ายังไม่หมดอายุ
      .single();
    ...
    // ส่ง Serial Key กลับไปให้ผู้ใช้ทางแชทไลน์
...
app.post('/verify-serial-key', express.json(), async (req, res) => {
  try {
    ...
    // บันทึก Machine ID และ IP Address ถ้ามี
    if (machineId || ipAddress) {
      console.log('Machine Info:', { machineId, ipAddress });
    }

    // ค้นหา Serial Key ในฐานข้อมูล
